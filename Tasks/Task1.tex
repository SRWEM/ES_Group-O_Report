...
\textbf{(a) Compute the annuity factor a(r,L) and the annualised costs for the technologies. \\
Output: Table (0.5)}
...
\textbf{(b) In the no-line case, what does the dispatch of solar and wind look like in each node? \\
...
\textbf{(c) Compare the two scenarios. How do the investments into generation, storage, and transmission change? \\
...
\textbf{(d) How does curtailment by technology and region change from the no-line case to the transmission case and why? \\
...
\textbf{(e) How do the residual load curves change across the two scenarios? \\
